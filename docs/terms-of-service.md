# Terms of Service

**VedAstro AI**
Last updated: 26 April 2026

By installing, accessing, or using VedAstro AI ("the app"), you agree to these Terms of Service. If you do not agree, please do not use the app.

For any questions: **support@vedastro.ai**

## 1. The service

VedAstro AI is an AI-powered Vedic astrology app. We provide:
- Personalized horoscopes (daily / weekly / monthly / yearly)
- AI guru chat for astrology questions
- Palm reading via photo upload
- Kundli / birth-chart analysis
- Subscription tiers for higher usage limits

The predictions are based on classical Vedic texts (BPHS, Phaladeepika, Brighu Sanhita) processed by Google Gemini. They are **for entertainment and reflective guidance only** — see Section 9 below.

## 2. Eligibility

You must be at least 13 years old to use VedAstro AI. To subscribe to any paid plan, you must be 18 or have parental / guardian consent and a valid Indian payment method.

## 3. Account

You're responsible for:
- Keeping your login credentials secure
- The accuracy of your profile information (name, DOB, place of birth — predictions depend on these)
- All activity under your account

You must NOT:
- Create accounts using false identity
- Share your account with others (each profile = one user)
- Attempt to bypass paywalls or quota limits
- Reverse-engineer the app or scrape the API

We may suspend or terminate accounts that violate these terms.

## 4. Subscription plans

We offer three plans (prices in INR):

| Plan | Price | Includes |
|------|-------|----------|
| Free 7-Day Trial | ₹0 today, ₹99/month from day 8 | 10 chats, 2 palm readings during trial |
| Standard | ₹199 / month | 30 chats + 5 palm readings per month |
| Premium | ₹499 / month | Unlimited chats + palm readings |

### 4.1 Free trial (Plan B — currently in effect)
- No charge today; e-mandate registered with your bank
- Some banks may briefly verify with a ₹1-5 charge, often refunded
- On day 8, ₹99 is auto-debited as the first paid month
- Subsequent months: ₹99 auto-debited every 30 days
- **You can cancel anytime during the 7-day trial → no charge ever happens**

### 4.2 Auto-renewal
All paid plans auto-renew monthly via Razorpay's e-mandate, until you cancel.

We will send you an SMS reminder 24 hours before each auto-debit, as required by RBI rules.

### 4.3 How to cancel
Anytime, from inside the app:
1. Settings → Subscription
2. Tap **Cancel Subscription**
3. Cancellation is immediate; you keep access until the end of the current paid period (e.g. you cancel on day 20, you keep access until day 30)

You may also email support@vedastro.ai or contact your bank to revoke the e-mandate.

### 4.4 Refund policy — no refunds

**All payments are final and non-refundable.**

When you cancel a paid plan:
- You keep premium access until the end of your current paid period
- No future auto-debit happens
- Past charges are NOT refunded

The free 7-day trial is the safest way to evaluate the app — you can cancel any time during the trial and you will never be charged.

**Exceptions** (handled case-by-case at our sole discretion):
- Demonstrable technical fault on our side that prevented the user from using paid features for the entire billing cycle
- Confirmed unauthorized transaction (you must also notify your bank for a chargeback investigation)

For exception requests: email support@vedastro.ai with your registered phone/email, the charge date, and details. We respond within 3 working days but make no commitment to refund.

### 4.5 Failed payments
If your bank declines a recurring debit (insufficient funds, expired card, etc.):
- Razorpay retries up to 4 times over 7 days
- We notify you by email + SMS
- If all retries fail, your subscription is suspended (premium turned off)
- You can re-subscribe anytime by tapping any premium feature

### 4.6 Price changes
We may change subscription prices for new users. Existing subscribers will be notified at least 30 days before any price change applies to their account; you may cancel before the new price takes effect.

## 5. Acceptable use

You agree NOT to:
- Use the app for unlawful purposes
- Submit false birth details to mislead other users (e.g. via shared family profiles)
- Spam the chat with abusive language, graphic violence, or harassment
- Upload images that are not palm photos (we use Vision AI which may flag misuse)
- Attempt to extract proprietary AI prompts or system instructions
- Submit content that infringes others' copyright
- Use the app to provide professional advice to third parties (legal, medical, financial)

We reserve the right to restrict accounts that abuse the service.

## 6. Intellectual property

- VedAstro AI app, branding, custom UI, AI prompts, and curated knowledge base: ©  VedAstro AI, all rights reserved
- Sanskrit texts (BPHS, Phaladeepika, Brighu Sanhita): public domain, attributed where shown
- AI-generated predictions: you may use them for personal reference; you may NOT republish them as your own paid content
- App content, screenshots, and trade names may not be copied without written permission

## 7. Privacy

Your privacy is governed by our [Privacy Policy](privacy-policy.md), which forms part of these Terms.

## 8. Third-party services

The app integrates:
- **Google Firebase** (authentication + database)
- **Google Gemini** (AI predictions)
- **Razorpay** (subscription payments)

Each of these has its own terms. By using VedAstro AI, you also agree to be bound by their respective terms. We are not responsible for outages, errors, or content from these third parties.

## 9. Disclaimer — predictions are NOT professional advice

The astrology readings, horoscopes, palm analysis, and AI guru responses provided by VedAstro AI are for **entertainment, cultural, and reflective purposes only**.

**They are NOT and shall not be relied upon as:**
- Medical advice or diagnosis
- Legal advice
- Financial or investment advice
- Career counseling certified by any board
- Mental health therapy
- Predictions of future events

For matters involving your health, finances, or legal status, please consult licensed professionals.

We make no warranty about the accuracy of any prediction.

## 10. Limitation of liability

To the maximum extent permitted by law:
- VedAstro AI is provided "AS IS" without warranties of any kind
- We are not liable for any indirect, incidental, consequential, or punitive damages arising from your use of the app
- Our total cumulative liability to you shall not exceed the amount you paid us in the 12 months prior to the claim

This does not limit liability for fraud or for matters that cannot be limited under Indian law.

## 11. Account suspension and termination

We may suspend or terminate your account if you:
- Violate these Terms
- Engage in fraudulent payment activity
- Abuse the service or other users
- Receive multiple chargebacks

We will notify you and provide reason. You may appeal by emailing support@vedastro.ai.

You may terminate at any time by deleting your account in Settings.

## 12. Force majeure

We are not liable for delays or non-performance caused by events outside our reasonable control, including but not limited to natural disasters, government action, internet outages, or third-party service failures (Firebase, Razorpay, Gemini).

## 13. Changes to these Terms

We may update these Terms as the service evolves. Material changes will be announced in-app and by email at least 30 days before taking effect. Continued use after the effective date means you accept the new Terms.

## 14. Governing law and dispute resolution

These Terms are governed by the laws of the Republic of India. Any dispute will be resolved through:

1. **Good-faith negotiation** by emailing support@vedastro.ai
2. If unresolved within 30 days, **arbitration** under the Arbitration and Conciliation Act, 1996, in [Your city], with a sole arbitrator
3. Subject to the above, exclusive **jurisdiction** of courts in [Your city], India

## 15. Contact

**Email:** support@vedastro.ai
**Operator:** [Your registered name / business entity]
**Address:** [Your registered address]
